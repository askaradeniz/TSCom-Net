...
def total_variation_loss(image, bmask):
    # shift one pixel and get difference (for both x and y direction)
    # loss = torch.mean(torch.abs(image[:, :, :, :-1] - image[:, :, :, 1:])) + \
    #      torch.mean(torch.abs(image[:, :, :-1, :] - image[:, :, 1:, :]))
    diff1 = torch.mean(torch.abs(image[:, :, :, :-1] - image[:, :, :, 1:])*bmask[:, :, :, :-1]) 
    diff2 = torch.mean(torch.abs(image[:, :, :-1, :] - image[:, :, 1:, :])*bmask[:, :, :-1, :])
    loss = diff1 + diff2
    return loss

# def dialation_holes(hole_mask):
#     b, ch, h, w = hole_mask.shape
#     dilation_conv = nn.Conv2d(ch, ch, 3, padding=1, bias=False).to(hole_mask.device)
...
class InpaintingLoss(nn.Module):
    ...
    def forward(self, input, mask, bmask, output, gt):
        loss_dict = {}
        output = bmask * output
        ...
        loss_dict['tv'] = total_variation_loss(output_comp, bmask)
