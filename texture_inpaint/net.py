...
class PartialConv(nn.Module):
    ...
    def forward(self, input, mask, bmask):
        ...
        # bmask_sum = output_bmask.masked_fill_(no_update_bholes, 1.0)
        # mask_sum = mask_sum + bmask_sum
...
class PConvUNet(nn.Module):
    ...
    def forward(self, input, input_mask, input_bmask):
        ...
        for i in range(self.layer_size, 0, -1):
            ...
            #enc_h_key_prev = 'h_{:d}'.format(i - 2)

            h = F.interpolate(h, scale_factor=2, mode=self.upsampling_mode)
            h_mask = F.interpolate(
                h_mask, scale_factor=2, mode='nearest')
            h_bmask = F.interpolate(
               h_bmask, scale_factor=2, mode=self.upsampling_mode)
            h = torch.cat([h, h_dict[enc_h_key]], dim=1)
            h_mask = torch.cat([h_mask, h_mask_dict[enc_h_key]], dim=1)
            h_bmask = h_bmask_dict[enc_h_key][:,:1,:,:].repeat(1,h_mask.size()[1],1,1) # 
