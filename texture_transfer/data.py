...
def load_landmarks_from_dir(scan_dir):
    path = pathlib.Path(scan_dir) / "landmarks3d.txt"
    if not path.exists():
        raise ValueError(f"landmarks not found {path}")
    return read_3d_landmarks(str(path))
...
def load_npz(path):
    ...
    # print(data['texture'].all() is None)
    if  data['texture'].all() is not None:
