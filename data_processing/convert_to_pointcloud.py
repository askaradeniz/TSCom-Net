...
def voxelized_pointcloud_sampling(partial_mesh_path):
    try:
        ...
        out_file = os.path.dirname(partial_mesh_path) + '/{}_voxelized_point_cloud.npz'\
        ...
        # sample from partial input
        partial_mesh = utils.as_mesh(trimesh.load(partial_mesh_path))
        point_cloud = partial_mesh.sample(num_points)
        ...
        np.savez(out_file, point_cloud=point_cloud, S = S, bbox = bbox, res = res)
...
def voxelized_colored_pointcloud_sampling(partial_mesh_path):
    try:
        ...
        out_file = os.path.dirname(partial_mesh_path) + '/{}_voxelized_colored_point_cloud.npz'\
        ...
        S = np.zeros(len(grid_points), dtype=np.int16)

        _, idx = kdtree.query(colored_point_cloud)
        _, idx_shape = kdtree.query(point_cloud)
        R[idx] = colors[:,0]
        G[idx] = colors[:,1]
        B[idx] = colors[:,2]
        S[idx_shape] = 1
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Generates the input for the network: a partial voxelized shape.'
    )
